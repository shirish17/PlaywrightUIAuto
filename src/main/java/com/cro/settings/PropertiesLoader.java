...
public final class PropertiesLoader {
	...
	private static final String DEFAULT_BROWSER = "chrome"; // fallabck logic --> If no browser is found in properties,
															// your loader silently falls back to chrome.
	...
	/**
	 * A Properties variant that stores keys in lowercase (Locale.ROOT) so that all
	 * key lookups become case-insensitive.
	 */
	private static final class CaseInsensitiveProperties extends Properties {
		...
		// stringPropertyNames() and propertyNames() will reflect normalized keys via
		// super
		@Override
	...
	public static Properties load() throws IOException {
		...
		// 3) Fallbacks (in exact order), first resolve env if not found comeout (check
		// in app-default), if found read URL, URL empty throw error
		...
		throw new FileNotFoundException(
				"Environment not supplied and default configs not found or missing 'env'. Tried: "
				...
				try {
					...
					if (testCachedDefaultProperties != null) {
						...
						if (isNonBlank(fallbackEnv)) {
							...
							System.out.println("[PropertiesLoader] Loaded properties from fallback: "
									+ TEST_DEFAULT_ENV_FILE + " (env=" + fallbackEnv + ", base.url=" + baseUrl + ")");
					...
					if (mainCachedDefaultProperties != null) {
						...
						if (isNonBlank(fallbackEnv)) {
							...
							System.out.println("[PropertiesLoader] Loaded properties from fallback: "
									+ MAIN_DEFAULT_ENV_FILE + " (env=" + fallbackEnv + ", base.url=" + baseUrl + ")");
							return mainCachedDefaultProperties;
						}
					}

					// If neither fallback works, throw
					String testMsg = (testCachedDefaultProperties == null) ? " (not found)"
							: " (found but missing 'env/base.url')";
					String mainMsg = (mainCachedDefaultProperties == null) ? " (not found)"
							: " (found but missing 'env/base.url')";

					throw new FileNotFoundException("Unable to resolve environment configuration!\n"
	...
	private static Properties loadFromUrlOrFileSpec(String spec) throws IOException {
		try {
			...
			try (InputStream in = url.openStream(); Reader r = new InputStreamReader(in, StandardCharsets.UTF_8)) {
	...
	/**
	 * Returns a prefix like "[ClassName.methodName] " for use in exception
	 * messages.
	 */
	...
	public static String effectiveBrowser() throws IOException {
		...
		if (isNonBlank(sys))
			return Browser.fromStringStrict(sys).toString();

		String env = getEnvVarIgnoreCase(ENVVAR_BROWSER); // case-insensitive env var
		if (isNonBlank(env))
			return Browser.fromStringStrict(env).toString();
	...
	// ===== NEW ===== Generic, reusable case-insensitive lookups for System props &
	// env vars

	/**
	 * Case-insensitive lookup of a JVM system property name. Supports -Denv=...,
	 * -DENV=..., -DeNv=..., etc.
	 */
	...
	/**
	 * Case-insensitive lookup of an environment variable name. Useful across
	 * platforms; on Linux env names are case-sensitive, but we opt-in to "best
	 * effort" scanning for consistency.
	 */
	...
	public static String getRequiredPropertyCached(String key) throws IOException {
		...
		if (value == null || value.trim().isEmpty()) {
			throw new IllegalStateException("Missing required property: '" + key + "' for env=" + resolveEnvStrict());
		}
		return value.trim();
	}
	...
	public static String getTenantName() throws IOException {
		String strTenantName=getRequiredPropertyCached("tenant.name");
		return normalizeEnv(strTenantName);
	}
	
	
	public static int getPageTimeout() throws IOException {
	    String value = getRequiredPropertyCached("page.load.timeout");
	    try {
	        return Integer.parseInt(value);
	    } catch (NumberFormatException e) {
	        throw new IllegalStateException(
	            "Invalid timeout value: '" + value + "' for page.load.timeout in properties file.", e);
	    }
	}
	
	public static int getElementTimeout() throws IOException {
	    String value = getRequiredPropertyCached("element.timeout");
	    try {
	        return Integer.parseInt(value);
	    } catch (NumberFormatException e) {
	        throw new IllegalStateException(
	            "Invalid timeout value: '" + value + "' for object.timeout", e);
	    }
	}
}
