...
public class UIActions {
 ...
 // ==================== LOCATOR CREATION METHODS ====================
     ...
     * @param selector - CSS selector, text selector or any other selector strategy provided by playwright
     */
    ...
    /*
	 * This method will click directly on the element by considering locator strategy
	 */
 ...
 // =========== ADDED METHODS FOR LOGIN EMAIL VALIDATION ===========
    ...
    public void clickByRole(AriaRole role, String name) {
        try {
            ...
            throw new RuntimeException(
                "Failed to click element with role " + role + " and name: " + name, e
            );
        } catch (Exception e) {
    ...
    //=========== unique method to select tenant in case of multi tenant application ===============
    /*
     * This method will look for the tenant on the 'Choose Account' popup and based on provided tenant name it will click the relevant 'Choose' button
     */
	public void selectTenant(String tenantPopup, String tenantName) {
		try {
			...
			Locator tenantSpan = chooseAccountPopup.locator("span", new Locator.LocatorOptions().setHasText(tenantName));
	...
	//=========== unique method to click on element using java script, needed if other locator strategies are not stable ====================
    /**
     * Execute JavaScript and wait for URL to change (for scripts that trigger navigation)
    ...
    public void executeScriptAndWaitForNavigation(String script) {
        try {
            ...
            page().waitForURL(
                url -> !url.equals(currentUrl),
                new Page.WaitForURLOptions().setTimeout(PageTimeout)
            );
    ...
    public void waitForFunction(String script) {
        try {
            page().waitForFunction(script, null, 
                new Page.WaitForFunctionOptions().setTimeout(ElementTimeout));
        } catch (TimeoutError e) {
  ...
  //============= UI Element visibility strategy ==================
     ...
     * Waits for element to appear before checking
     * @param selector - CSS selector
  ...
  //============= Kendo strategies ==================
     ...
     * Use this for complex operations or page loads
     */
    public void waitForAllKendoLoadersComplete() {
        ...
        try {
            ...
            try {
                page().locator(loaderSelector).first().waitFor(
                    new Locator.WaitForOptions()
